...
length(unique(briofCan$speciesKey)) # how many unique species names
length(unique(briofCan$scientificName)) # how many unique species codes
```
...
### Where can I get data to limit the R.O.I. ? 
...
```{r}
# Build the species site matrix
gridTab <- table(briofCanClean$unID,
                 briofCanClean$speciesKey )
# separate effort vs presence absence. 
...
```{r}
# remove species and grids with little information 

SpTab2  <- gridTab[rowSums(gridTab) > 10,
                 -which(colSums(gridTab) 
                        %in% c(1:2))]
# compute the distance matrix of the new prunned matrix
